/* DFA for strings having 001 as a substring
...
int main(){
    ...
    cout << "DFA: Accept strings having 001 as a substring\n";
    cout << "Enter string: ";
    cin >> str;
    cout << "\nTransition: q0";
    ...
    for(size_t i = 0; i < str.length(); i++){ //size_t: an unsigned int type used for rep.sizes and counts
        ...
        else if(state == 1 && str[i] == '1') state = 0;
        else if(state == 2 && str[i] == '0') state = 2;
        else if(state == 2 && str[i] == '1') state = 3;
        else if(state == 3 && (str[i]=='0' || str[i]=='1')) state = 3;
